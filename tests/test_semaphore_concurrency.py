...
class TestSemaphoreConcurrency:
    ...
    async def test_single_semaphore_limits_concurrency(
        self, task_manager, tracked_execute_cpu
    ):
    ...
    @pytest_asyncio.mark.asyncio
    async def test_multiple_semaphore_allows_concurrency(
        self, task_manager, tracked_execute_io
    ):
    ...
    @pytest_asyncio.mark.asyncio
    async def test_default_semaphore_behavior(self, task_manager):
    ...
    @pytest_asyncio.mark.asyncio
    async def test_task_status_flow(self, task_manager):
        ...
        assert (
            status_changes[0][1] == "running"
        )  # Should be running when execute is called

    @pytest_asyncio.mark.asyncio
    async def test_semaphore_limits_respected(self, task_manager, long_running_execute):
